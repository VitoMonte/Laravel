...
@if($article)
    <div class="hentry hentry-post blog-big group ">
        ...
        <div class="meta group">
            ...
            <p class="categories"><span>In: <a href="{{ route('articlesCat', ['cat_alias' => $article->category->alias])}}" title="{{ Lang::get('ru.view_all_posts_in') }} {{$article->category->title}}" rel="category tag">{{$article->category->title}}</a></span></p>
            <p class="comments"><span><a href="#comments" title="">{{ count($article->comments) >0 ? count($article->comments) : Lang::get('ru.no_comments') }}</a></span></p>
        </div>
        <!-- post content -->
    ...
    <div id="comments">
        ...
        <div id="respond">
            <h3 id="reply-title"> {{ Lang::get('ru.leave_a') }}<span> {{ Lang::get('ru.is_replay') }}</span> <small><a rel="nofollow" id="cancel-comment-reply-link" href="#respond" style="display:none;"> / {{Lang::get('ru.cancel_replay') }}</a></small></h3>
            <form action="{{ route('comment.store') }}" method="post" id="commentform">            
                ...
                <div class="clear"></div>
                <p class="form-submit">
                    <input id="comment_post_ID" name="comment_post_ID" type="hidden" value="{{$article->id}}" />
